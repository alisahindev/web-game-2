...
const colorsForLevel = (id: number): CrystalColor[] => {
  if (id <= 10) return ["amber", "lava", "ice", "moss"];
...
const symbolFor = (row: number, col: number, id: number, colors: CrystalColor[]): string => {
  if (row === 0) return colorSymbols[(col + id) % colors.length];
  ...
  return colorSymbols[(row * 2 + col + id) % colors.length];
};
...
const makeLevel = (id: number): Level => {
  ...
  return {
    ...
    rows: 11,
    cols: 8,
    shots: Math.max(12, 20 - Math.floor(id / 5)),
    colors,
